...
async function sendOtp(isResend = false) {
  ...
  loading.value = true
  message.value = ''
  // shouldCreateUser: true 鈫?first OTP both registers and signs in
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div class="row" style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <button class="btn secondary" type="button" @click="hideLogin">脳</button>
        </div>
        ...
        <p v-if="message" class="login-msg" :class="{ error: /fail|incorrect|error|澶辫触|閿欒/i.test(message) }">{{ message }}</p>
      </div>
    </div>
  </div>
</template>
